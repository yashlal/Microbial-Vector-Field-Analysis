...
def main(plots='stack'):
    ...
    combined_library = custom_library + fourier_library + polynomial_library

    trapp_opt = ps.TrappingSR3(threshold=0.075, max_iter=100)
    model = ps.SINDy(optimizer=trapp_opt, feature_library=combined_library, discrete_time=True)
    model.fit(traj_train_multi, multiple_trajectories=True, t=t_train_multi)
    model.print()
