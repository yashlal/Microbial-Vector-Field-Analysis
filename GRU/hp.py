...
def hp_wrapper(config):
    ...
    for test_ind in test_inds:
        ...
        best_epoch, ensemble_indiv_loss = gru_funcs.all_loss_train(num_channels=15, hsize=hidden_layer_size, layers=num_layers, dropout=dropout, batch_size=batch_size, LR=LR, num_epochs=1000, train_dataloader=train_dataloader, test_in=test_in, tensor_true_test_data=tensor_true_test_data, device=device, return_best=True)
...
if __name__=='__main__':
    ...
    with mp.Pool(processes=12) as pool:
        pool.map(hp_wrapper, search_space)
